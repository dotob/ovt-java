...
public class MFErgebnisPanel extends JPanel implements ActionListener, ChartMouseListener {

	private static final long serialVersionUID = 438918593224717613L;
	private static final String name = "MF-Ergebnisse";
	...
	private Component plzTxt;
	private JTextField plzInput;
	private IMainWindow parentWindow;

	public MFErgebnisPanel(IMainWindow pw) {
	...
	private void initGUI() {
		...
		JLabel mfTxt = new JLabel("Marktforscher:");
		mf.add(mfTxt);
	...
	private void collectData() {
		...
		try {
			...
			ResultSet rsAll = Database.select("count(*)", "kunden k, aktionen a, ergebnisse e", "WHERE "
					+ "k.id=a.kunde AND a.ergebnis=e.id AND e.finishedafter=1 AND a.angelegt>='" + range.getVon()
					+ "' AND a.angelegt<='" + range.getBis() + "' " + this.getMaFoSQL() + this.getPLZSQL());
			if (rsAll.next()) {
			...
			ResultSet rs = Database
					.select(
							"e.gruppe, eg.name, count(e.gruppe)",
							"kunden k, aktionen a, ergebnisse e, ergebnisgruppen eg",
							"WHERE "
									+ "k.id=a.kunde AND a.ergebnis=e.id AND eg.id=e.gruppe AND e.finishedafter=1 AND a.angelegt>='"
									+ range.getVon() + "' AND a.angelegt<='" + range.getBis() + "' "
									+ this.getMaFoSQL() + this.getPLZSQL() + " GROUP BY e.gruppe");
			while (rs.next()) {
				String grp = rs.getString("e.gruppe");
				int countGrp = rs.getInt(3);
				this.groupData.put(grp, countGrp);

				Vector<String> grpRowData = new Vector<String>();
				grpRowData.add(rs.getString(2));
				grpRowData.add("");
				...
				ResultSet rsErg = Database
						.select("a.ergebnis, count(a.ergebnis)", "kunden k, aktionen a, ergebnisse e", "WHERE "
								+ "k.id=a.kunde AND a.ergebnis=e.id AND e.finishedafter=1 AND a.angelegt>='"
								+ range.getVon() + "' AND e.gruppe=" + grp + " AND a.angelegt<='" + range.getBis()
								+ "' " + this.getMaFoSQL() + getPLZSQL() + " GROUP BY a.ergebnis");
				while (rsErg.next()) {
	...
	private void chartGrpErgData(String grp) {
		...
		DefaultPieDataset result = new DefaultPieDataset();
		ResultSet rsErg = Database.select(
											"a.ergebnis, count(a.ergebnis)",
											"kunden k, aktionen a, ergebnisse e",
											"WHERE k.id=a.kunde AND a.ergebnis=e.id AND e.finishedafter=1 AND a.angelegt>='"
	...
	private void chartErgData() {
		...
		ResultSet rsErg = Database.select(
											...
											"WHERE k.id=a.kunde AND a.ergebnis=e.id AND e.finishedafter=1 AND a.angelegt>='"
													+ range.getVon() + "' AND a.angelegt<='" + range.getBis() + "' "
													+ this.getMaFoSQL() + this.getPLZSQL() + " GROUP BY a.ergebnis");
		try {
			while (rsErg.next()) {
				String erg = rsErg.getString("a.ergebnis");
				int countErg = rsErg.getInt(2);
		...
		JFreeChart chart = ChartFactory.createPieChart("Anzahl bearbeiteter Adressen: " + mfName, // chart
														// title
														result, // data
														false, // include legend
														true,
	...
	private void chartMFData() {
		...
		DefaultPieDataset result = new DefaultPieDataset();
		ResultSet rsErg = Database.select(
											"a.marktforscher, count(a.marktforscher)",
											"kunden k, aktionen a, ergebnisse e",
											"WHERE k.id=a.kunde AND a.ergebnis=e.id AND e.finishedafter=1 AND a.angelegt>='"
													+ range.getVon() + "' AND a.angelegt<='" + range.getBis() + "' "
													+ this.getMaFoSQL() + this.getPLZSQL()
													+ " GROUP BY a.marktforscher");
		try {
			while (rsErg.next()) {
				String mfName = "unbekannt";
				String mf = rsErg.getString("a.marktforscher");
				int countErg = rsErg.getInt(2);
				Marktforscher mafo = Marktforscher.SearchMarktforscher(mf);
				if (mafo != null) {
					mfName = mafo.toString();
				} else {
					mfName += " (" + mf + ") = " + Integer.toString(countErg);
				}
	...
	private String getMaFoSQL() {
		...
		if (mf.trim().equals("allFT")) {
			...
			mafoSQL = " AND a.marktforscher=" + mf;
		}
		return mafoSQL;
	}
