...
public class MFGespraechPanel extends JPanel implements ActionListener {

	private static final long serialVersionUID = -22348298304355088L;
	private static final String name = "MF-Termine";
	...
	public MFGespraechPanel(IMainWindow pw) {
	...
	private String getMaFoSQL() {
		...
		if (mf.trim().equals("allFT")) {
			...
			mafoSQL = " AND g.marktforscher=" + mf;
		}
		return mafoSQL;
	}
	...
	private void chartMFData() {
		...
		if (mfl.trim().equals("allFT")) {
			...
			mafoSQL = " AND g.marktforscher=" + mfl;
		}

		DefaultPieDataset result = new DefaultPieDataset();
		ResultSet rsErg = Database.select("g.marktforscher, count(g.marktforscher)",
				"gespraeche g, terminergebnisse e", "WHERE " + "g.ergebnis=e.id AND g.datum_vd>='" + range.getVon()
						+ "' AND g.datum_vd<='" + range.getBis() + "' " + mafoSQL + " GROUP BY g.marktforscher");
		try {
			while (rsErg.next()) {
				String mf = rsErg.getString("g.marktforscher");
				Marktforscher mafo = Marktforscher.SearchMarktforscher(mf);
				if (mafo != null) {
					String mfName = mafo.toString();
					int countErg = rsErg.getInt(2);
		...
		JFreeChart chart = ChartFactory.createPieChart("Anzahl Termine aller Marktforscher", // chart
				// title
				result, // data
				false, // include legend
				true, false);
	...
	private void exportTableData() {
		...
		fields[2] = 2500;
		fields[3] = 2500;
		Table2Excel xls = new Table2Excel(this.mfeTable, "statisticsTMP.xls", "MF-Gesprächsergebnisse", fields);
		xls.openXLS();
	}
